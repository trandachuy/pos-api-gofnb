...
BEGIN 
	...
	DECLARE TABLE_RECORD_CURSOR CURSOR 
	  LOCAL STATIC READ_ONLY FORWARD_ONLY
	FOR
	SELECT Id FROM ProductCategory WHERE StoreId = @StoreId ORDER BY CreatedTime ASC
	...
	BEGIN
		...
		IF LEN(@Code) > 0
			UPDATE ProductCategory SET Code = @Code WHERE Id = @RecordId
	...
	IF EXISTS(SELECT 1 FROM [StoreConfig] WHERE StoreId = @StoreId)
		BEGIN
			UPDATE [StoreConfig] SET CurrentMaxProductCategoryCode = @CurrentIndex WHERE StoreId = @StoreId
		END
	ELSE
		BEGIN
			INSERT [StoreConfig] (
